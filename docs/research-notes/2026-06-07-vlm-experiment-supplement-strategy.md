...
- **InternVL2-8B**：审稿人最高概率质疑点。InternVL3 的 blog（2025年4月）在基准测试表中直接比较了 Qwen2.5-VL 和 InternVL2.5，展示代际差距。审稿人会问 "why not InternVL3?"
...
| **InternVL3-8B** | 升级替代 InternVL2-8B | — | — | ❌ 需要新 wrapper |

### 2.2 为什么是 InternVL3-8B 而非 InternVL3.5-8B

| 维度 | InternVL3-8B | InternVL3.5-8B |
|------|-------------|----------------|
| 发布时间 | 2025年4月 | 2025年8月 |
| 社区文档 | 完善（blog + 多篇解读） | 较新 |
| 推理代码 | 基于 InternVLChatModel，chat() API | 相同的 chat() API |
| HF 模型卡 | 详细 benchmark 表 | 有 |
| 与 InternVL2 兼容性 | 相同架构家族，chat() API 一致 | 相同 |

**选择 InternVL3-8B 的理由**：InternVL3.5 的增量主要由 Cascade RL 后训练驱动（推理性能 +16%、推理速度 4×），对 captioning 任务的影响可能有限。InternVL3 是更保守、更稳妥的选择 — 它的 2025年4月发布日期意味着审稿人不能说你用了「过时」的模型。如果 InternVL3.5 在 InternVL2→InternVL3 wrapper 的基础上 zero-cost 地 work，则可以直接使用 InternVL3.5。
...
已有 LoRA 配置 (config/training.py):
  ...
  internvl3           ✅ 4 modules, r=16 — 配置已存在但无 wrapper
  internvl35          ✅ 4 modules, r=16 — 配置已存在但无 wrapper
...
| 任务 | 复杂度 | 预计时间 | 依赖 |
|------|--------|----------|------|
| `models/internvl3.py` wrapper | 低 | 0.5h | `_internvl_base.py` 可直接复用 |
| `config/__init__.py` 添加 internvl3 | 低 | 5min | wrapper 就绪 |
| 验证 InternVL3 ZS 推理 | 低 | 0.5h | wrapper + HF 模型已下载 |
...
| 验证 InternVL3 QLoRA 加载 | 中 | 1h | training.py 配置已存在 |

### 3.3 InternVL3 Wrapper 设计

InternVL3 使用与 InternVL2 相同的 `chat()` API（`InternVLChatModel`），架构继承关系为：
...
因此 `_internvl_base.py` 可以直接复用。InternVL3 wrapper 仅需：

```python
class InternVL3Wrapper(InternVLBase):
...
与 InternVL2 的唯一差异：
- InternVL3 的 LLM backbone 从 InternLM2-7B 变为 Qwen2.5-7B
- Tokenizer 从 `InternLM2Tokenizer` 变为 Qwen2.5 的 tokenizer
- 这意味 `_load_tokenizer()` 的路径可能需要调整（不再需要 `sys.path` 加载 `tokenization_internlm2`）
- 可能需要改为直接使用 `AutoTokenizer.from_pretrained()`

**需要验证的关键点**：InternVL3 的 chat() 函数签名是否与 InternVL2 完全相同（pixel_values + question + generation_config）。
...
```
Phase 0 — 补齐工程（新增）
  ├─ 创建 InternVL3 wrapper
  ├─ 注册到 MODEL_REGISTRY
  ├─ 确认 Qwen3-VL-8B 和 InternVL3-8B 模型文件已下载
...
Phase 2 — Zero-Shot 全覆盖（模型名单更新）
  └─ instructblip + llava + qwen3vl + internvl3 × Prompt A × 3500 images
     （如果原 internvl2 和 qwen2vl 的 ZS 结果已经跑完，可保留作为
      supplementary table 中的比较数据，证明「代际提升」）

Phase 3 — Few-Shot（模型名单更新）
  └─ llava + qwen3vl × k=1,3 × 3500 images
     （Qwen2.5-VL few-shot 结果可保留在 supplementary 中）
...
> "Even the latest generation of VLMs (InternVL3, Qwen3-VL) — which substantially outperform their predecessors on general benchmarks — struggle on this specialized domain, reinforcing the need for domain-specific approaches."
...
- InternVL2-8B 的 ZS 结果 → 证明 InternVL2 → InternVL3 的代际进步

**审稿人 value**：如果你能展示 "InternVL3 在 MMMU 上比 InternVL2 高 10+ 分，但在 UICO captioning 上只高了 X 分"，这本身就是一个 contribution — 说明通用 benchmark 的提升不一定迁移到 specialized domain。
...
| 风险 | 概率 | 影响 | 应对 |
|------|------|------|------|
| InternVL3 chat() API 不兼容 | 低 | 中 | fallback 到 InternVL2.5（已有 wrapper）或手动构建 processor 管线 |
...
| InternVL3 8B 显存不足 | 低 | 高 | InternVL3-8B 参数量与 InternVL2-8B 接近，InternVL2-8B 已在 24GB 上通过 |
...
```
Tier 1 (目标): LLaVA-1.5 + Qwen3-VL-8B + InternVL3-8B + InstructBLIP
Tier 2 (InternVL 降级): LLaVA-1.5 + Qwen3-VL-8B + InternVL2.5-8B + InstructBLIP
                       （InternVL3 chat() API 不兼容时的降级方案）
Tier 3 (Qwen 降级):    LLaVA-1.5 + Qwen2.5-VL-7B + InternVL3-8B + InstructBLIP
...
InternVL3-8B       | VLM (ZS)     | tbd  | tbd  | tbd  | tbd  | tbd  | tbd  | tbd
...
InternVL3-8B       | 2025  | 62.7           | tbd               | InternVL3 − InternVL2
```
...
```
Priority 1 — InternVL3 Wrapper + ZS 验证（最低风险、最高审稿收益）
  └─ 创建 wrapper、注册到 registry、验证推理
  └─ 时间: ~1h | 风险: 低
...
- 新增一段描述 VLM 发展的代际进步（LLaVA-1.5 → Qwen3-VL/InternVL3）
- 强调 "we select models spanning three generations of VLM development to provide a comprehensive baseline"
- 引用 InternVL3 blog (2025.4) 和 Qwen3-VL technical report (arXiv:2511.21631) 证明选择的模型代表了当前技术水平
...
- 明确每个模型的发布年份和选择理由：
  ...
  - InternVL3: "latest generation with strong reasoning capabilities (MMMU 62.7)"
...
| InternVL 模型 | InternVL2-8B | **InternVL3-8B** | 时效性：代际提升 +11.5 推理分 |
| 新增工程 | Qwen2VL LoRA | **InternVL3 wrapper** + Qwen3VL LoRA 验证 | Qwen3VL wrapper 已有，InternVL3 wrapper 需新建 |
...
| 风险 | 主要是 Qwen2VL LoRA NaN | Qwen3VL LoRA NaN + InternVL3 API 兼容 | bf16 修复降低了 NaN 风险 |
...
- 模型注册: `config/__init__.py`（需要添加 internvl3）
- Training 配置: `config/training.py`（internvl3/internvl35 配置已存在）
- Wrapper 基类: `models/_internvl_base.py`（InternVL3 可复用）
- 已有 wrapper: `models/internvl2.py`, `models/qwen3vl.py`, `models/qwen2vl.py`
