...
| **InternVL3.5-8B** | 升级替代 InternVL2-8B | — | — | ❌ 需要新 wrapper |
...
已有 wrapper (models/):
  ...
  internvl35.py       ✅ InternVL3.5-8B — ZS 可用（新增 wrapper）
...
| 验证 InternVL3.5 QLoRA 加载 | 中 | 1h | training.py 配置已存在 |
...
```
Priority 1 — InternVL3.5 Wrapper + ZS 验证（最低风险、最高审稿收益）
  └─ 创建 internvl35 wrapper（已完成）、验证推理
  └─ 时间: ~1h | 风险: 低
